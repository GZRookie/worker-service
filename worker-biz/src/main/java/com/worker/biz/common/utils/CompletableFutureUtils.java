...
public class CompletableFutureUtils {
    ...
    public static final int THREAD_CORE = Runtime.getRuntime().availableProcessors();

    static {
        initExecutor();
        Runtime.getRuntime().addShutdownHook(
                new Thread(CompletableFutureUtils::shutdown, "CompletableFutureThreadPool-ShutdownHook"));
    }
    ...
    private static void initExecutor() {
        if (executorService == null) {
            synchronized (CompletableFutureUtils.class) {
                if (executorService == null) {
                    executorService = new ThreadPoolExecutor(THREAD_CORE, THREAD_CORE * 3, 1, TimeUnit.MINUTES,
                            new ArrayBlockingQueue<>(10000),
                            new NamedThreadFactory("CompletableFutureFactory", true));
                }
            }
        }
    }
     ...
     * @return 任务结果
     */
    public static <T> CompletableFuture<T> submit(ExecutorService executorService, Supplier<T> task) {
